...
#Funcion que transforma el texto a un DF para Casos Confirmados
    ...
    def user_passw(self,us,pa):
        print('**** SE PROCEDE A IMPORTAR USUARIO Y CLAVE ****\n')
        self.user  = str(us)
        self.passw = str(pa)
        print('**** LISTO ****\n')
    
    def abre_pag(self,url):
        print('**** SE PROCEDE A ABRIR EL NAVEGADOR ****\n')
        self.driver.get(url)
        time.sleep(2)
        print('**** LISTO ****\n')
    ...
    def descargar_info(self):
        ...
        if dato=='NOTIF2':
            ...
            self.Tabla.head()
            print('')
            name=str(input('Ingrese el nombre que tendrá el archivo: '))
        ...
        #Hacer función pa las notif1
            ...
            for i in range(0,int(cant/100)):
                texto=self.driver.find_element(By.XPATH,'//*[@id="tabla_boletin_eno_nuevas"]').text
                #Aquí arreglar función para transformar texto a tabla!
                df=self.aTabla_por_notif(texto)
                self.Tabla=pd.concat([self.Tabla,df],ignore_index =True)
                self.siguiente()
            print('**** SE HA CREADO LA TABLA CON: ****',self.Tabla.shape[0],' ENTRADAS')
